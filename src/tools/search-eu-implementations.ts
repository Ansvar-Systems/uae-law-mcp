/**
 * search_eu_implementations — Search EU directives/regulations that have
 * UAE implementing/aligning legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_uae_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as uae_statute_count
  ...
  if (input.has_uae_implementation) {
    sql += ' HAVING uae_statute_count > 0';
  }
