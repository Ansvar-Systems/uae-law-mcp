/**
 * validate_eu_compliance — Check EU/international alignment status for a
 * UAE statute, DIFC law, or ADGM regulation.
 */
...
export async function validateEUCompliance(
  ...
  ).get(resolvedId) as { id: string; title: string; status: string; legal_zone: string | null };
  ...
  if (euRefCount === 0) {
    return {
      results: {
        ...
        legal_zone: doc.legal_zone,
        warnings: [],
        recommendations: ['No EU/international cross-references found for this law. This may be a purely domestic statute.'],
      },
      _metadata: generateResponseMetadata(db),
    };
  }
