#!/usr/bin/env tsx
/**
 * UAE Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://elaws.moj.gov.ae';
const PORTAL_NAME = 'UAE Ministry of Justice e-Laws';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/uae-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('UAE Law MCP — Data Freshness Check');
