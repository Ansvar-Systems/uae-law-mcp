#!/usr/bin/env tsx
/**
 * Drift detection for UAE Law MCP.
 *
 * Checks if upstream content from moj.gov.ae, difclaws.com, and adgm.com
 * has changed since last ingestion. Uses the golden-hashes.json fixture
...
async function main(): Promise<void> {
  console.log('UAE Law MCP -- Drift Detection');
  console.log('==============================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'UAELawMCP/1.0 drift-detect' },
      });
