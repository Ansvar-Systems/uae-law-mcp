#!/usr/bin/env tsx
/**
 * Database builder for UAE Law MCP server.
 *
 * Builds the SQLite database from seed JSON files in data/seed/.
 * UAE-specific: includes legal_zone and language columns for the three-layer system.
 *
...
// Database schema — UAE-specific with legal_zone and language columns
const SCHEMA = `
-- Legal documents (statutes, regulations, decree-laws)
CREATE TABLE legal_documents (
  ...
  url TEXT,
  description TEXT,
  legal_zone TEXT CHECK(legal_zone IN ('federal', 'difc', 'adgm')),
  language TEXT DEFAULT 'en',
  last_updated TEXT DEFAULT (datetime('now'))
);

CREATE INDEX idx_documents_zone ON legal_documents(legal_zone);
...
-- FTS5 for provision search (unicode61 tokenizer supports Arabic)
...
function buildDatabase(): void {
  console.log('Building UAE Law MCP database...\n');
  ...
  const insertDoc = db.prepare(`
    INSERT INTO legal_documents (id, type, title, title_en, short_name, status, issued_date, in_force_date, url, description, legal_zone, language)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const loadAll = db.transaction(() => {
    for (const file of seedFiles) {
      ...
      insertDoc.run(
        ...
        seed.legal_zone ?? 'federal', seed.language ?? 'en',
      );
      totalDocs++;
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from UAE statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'AE');
    insertMeta.run('source', 'moj.gov.ae, difclaws.com, adgm.com');
